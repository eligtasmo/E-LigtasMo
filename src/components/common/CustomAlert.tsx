...
const CustomAlert: React.FC<CustomAlertProps> = ({
  ...
  return (
    <div className={`rounded-lg border p-4 ${variantClasses[variant].container} ${className}`}>
      <div className="flex items-start gap-3">
        <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${variantClasses[variant].iconBg}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          {showLink && (
            <Link
              to={linkHref}
              className="inline-block mt-2 text-sm font-medium text-gray-700 underline hover:text-gray-900 dark:text-gray-300 dark:hover:text-white"
            >
              {linkText}
            </Link>
          )}

...
import { View, Text, TouchableOpacity, useWindowDimensions, Platform, StyleSheet, ScrollView } from 'react-native';
...
export const NavigationOverlay = ({
  ...
  const getDetailedInstruction = () => {
    ...
    if (!base || base.trim() === '') {
      return "Follow the highlighted tactical path";
    }
  ...
  const arrivalText = navigationData?.arrivalTimestamp ? new Date(navigationData.arrivalTimestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--';
  const etaText = `${Math.ceil((navigationData?.durationRemaining || 0) / 60)} min`;
  const remainingText = `${((navigationData?.distanceRemaining || 0) / 1000).toFixed(1)} km`;
              ...
              <Row align="center" style={{ marginTop: isExpanded ? 0 : 252 }}>
                ...
                <Col style={{ flex: 1, marginLeft: 16 }}>
                  ...
                  <Col style={{ marginTop: 2 }}>
                    ...
                    {upcomingSteps.length > 0 && !isExpanded && (
                       <Text style={{ fontSize: 10, fontWeight: '700', color: BLUE, marginTop: 2, textTransform: 'uppercase' }}>
                         Then: {upcomingSteps[0].instruction}
                       </Text>
                    )}
                  </Col>
                </Col>
                    ...
                    {upcomingSteps.map((step, idx) => {
                      ...
                      return (
                        <Row key={idx} align="center" gap={12} style={{ marginBottom: 12 }}>
                          ...
                          <Col style={{ flex: 1 }}>
                            <Text style={{ fontSize: 13, fontWeight: '600', color: '#FFFFFF' }} numberOfLines={2}>{step.instruction}</Text>
                            <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{Math.round(step.distance || step.distanceMeters || 0)} m</Text>
                          </Col>
                        </Row>
                      );
                    })}
        ...
        {(nearestBarangay || nearestShelter) && (
          ...
          >
            {nearestBarangay && (
              <TouchableOpacity 
                activeOpacity={0.8}
                onPress={() => onRedirect(nearestBarangay, nearestBarangay.name)}
            ...
            {nearestShelter && (
              <TouchableOpacity 
                activeOpacity={0.8}
                onPress={() => onRedirect(nearestShelter, nearestShelter.name)}
                style={{ backgroundColor: 'rgba(21,17,14,0.94)', padding: 14, borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', width: 160, minWidth: 160 }}
              >
                <Row align="center" gap={6} style={{ marginBottom: 4 }}>
  ...
  return (
    <View style={[atomic.l.abs, atomic.l.fill, { zIndex: 3000, pointerEvents: 'box-none' }]}>
      ...
      >
        <View style={{
          ...
          <TouchableOpacity 
            ...
            style={{ padding: 20, flexDirection: 'row', alignItems: 'center', minHeight: 92 }}
          >
            ...
            <Col style={{ flex: 1, marginLeft: 16 }}>
              ...
              <Col style={{ marginTop: 2 }}>
                ...
                {upcomingSteps.length > 0 && !isExpanded && (
                   <Text style={{ fontSize: 11, fontWeight: '600', color: BLUE, marginTop: 4, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                     Then: {upcomingSteps[0].instruction}
                   </Text>
                )}
              </Col>
          ...
          {isExpanded && (
            ...
            >
              <View style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.08)', marginBottom: 16 }} />
              <Row align="center" justify="between" style={{ marginBottom: 12, paddingHorizontal: 4 }}>
                <Text style={{ fontSize: 11, fontWeight: '800', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: 1 }}>Mission Itinerary</Text>
                <Text style={{ fontSize: 10, fontWeight: '600', color: 'rgba(255,255,255,0.2)' }}>{upcomingSteps.length} steps remaining</Text>
              </Row>
              <ScrollView 
              ...
              >
                {upcomingSteps.map((step, idx) => {
                  const StepIcon = getManeuverIcon(step.instruction);
                  const stepDist = step.distance || step.distanceMeters || 0;
                  return (
                    <View key={idx} style={{ marginBottom: 16 }}>
                      <Row align="center" gap={16}>
                        <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.04)', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: 'rgba(255,255,255,0.05)' }}>
                          <StepIcon size={16} color={idx === 0 ? BLUE : "rgba(255,255,255,0.6)"} strokeWidth={2.5} />
                        </View>
                        <Col style={{ flex: 1 }}>
                          <Text style={{ fontSize: 14, fontWeight: '600', color: '#FFFFFF', lineHeight: 18 }}>{step.instruction}</Text>
                          <Row align="center" gap={4} style={{ marginTop: 2 }}>
                            <Lucide.Navigation2 size={10} color="rgba(255,255,255,0.3)" />
                            <Text style={{ fontSize: 11, fontWeight: '700', color: 'rgba(255,255,255,0.4)', letterSpacing: 0.2 }}>
                              {stepDist >= 1000 ? `${(stepDist/1000).toFixed(1)} km` : `${Math.round(stepDist)} m`}
                            </Text>
                          </Row>
                        </Col>
                      </Row>
                      {idx < upcomingSteps.length - 1 && (
                        <View style={{ width: 2, height: 8, backgroundColor: 'rgba(255,255,255,0.05)', marginLeft: 15, marginTop: 4 }} />
                      )}
                    </View>
                  );
                })}
      ...
      <MotiView 
        ...
        animate={{ opacity: 1, translateY: 0 }} 
        style={{ position: 'absolute', left: 16, right: 16, bottom: safeBottom, zIndex: 3200 }}
      >
        <View style={{ 
          width: '100%', 
          ...
          >
            <Lucide.X size={26} color="#EF4444" strokeWidth={3} />
          </TouchableOpacity>

          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', paddingHorizontal: 10 }}>
            <Col align="center">
              <Text style={{ fontSize: 24, fontWeight: '900', color: '#FFFFFF', letterSpacing: -1.2 }}>{etaText.replace(' min', '')}</Text>
              <Text style={{ fontSize: 9, fontWeight: '800', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 1.2 }}>MIN</Text>
            </Col>
            <View style={{ width: 1.5, height: 28, backgroundColor: 'rgba(255,255,255,0.1)' }} />
            <Col align="center">
              <Text style={{ fontSize: 24, fontWeight: '900', color: BLUE, letterSpacing: -1.2 }}>{remainingText.replace(' km', '')}</Text>
              <Text style={{ fontSize: 9, fontWeight: '800', color: 'rgba(47, 123, 255, 0.6)', textTransform: 'uppercase', letterSpacing: 1.2 }}>KM</Text>
            </Col>
            <View style={{ width: 1.5, height: 28, backgroundColor: 'rgba(255,255,255,0.1)' }} />
            <Col align="center">
              <Text style={{ fontSize: 22, fontWeight: '900', color: 'rgba(255,255,255,0.95)', letterSpacing: -0.8 }}>{arrivalText}</Text>
              <Text style={{ fontSize: 9, fontWeight: '800', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: 1.2 }}>ARR</Text>
            </Col>
          </View>
          ...
          >
            <Lucide.Compass size={26} color="#FFFFFF" strokeWidth={2.5} />
          </TouchableOpacity>
        </View>
      </MotiView>
    </View>

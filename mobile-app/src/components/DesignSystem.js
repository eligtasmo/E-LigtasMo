...
export const SimulatorWrapper = ({ children }) => {
  ...
  const presets = {
    responsive: { w: windowWidth - 340, h: windowHeight - 100, label: 'RESPONSIVE' },
    phoneMedium: { w: 393, h: 852, label: '6.1" PHONE' },
    phoneLarge: { w: 430, h: 932, label: '6.7" PHONE' },
    tabletMedium: { w: 820, h: 1180, label: '10.9" TABLET' },
  };

  const isPortrait = orientation === 'portrait';
  const baseWidth = presets[preset].w;
  const baseHeight = presets[preset].h;

  const frameWidth = isPortrait ? baseWidth : baseHeight;
  const frameHeight = isPortrait ? baseHeight : baseWidth;

  return (
    <SimulatorContext.Provider value={{ width: frameWidth, height: frameHeight }}>
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: '#080808' }}>
        {/* Left Side: Tactical Control Sidebar */}
        <View style={{
          width: 300,
          backgroundColor: '#111',
          borderRightWidth: 1,
          ...
          <View>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 40 }}>
              <View style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: '#B37213', alignItems: 'center', justifyContent: 'center' }}>
                <LucideIcons.Shield size={20} color="#FFF" />
              </View>
              <View>
                <Text style={{ color: '#FFF', fontWeight: '700', fontSize: 16, letterSpacing: 1 }}>E-LigtasMo</Text>
                <Text style={{ color: '#666', fontWeight: '600', fontSize: 9, letterSpacing: 2 }}>TACTICAL HUB</Text>
              </View>
            </View>

            <Text style={{ color: '#444', fontWeight: '600', fontSize: 10, letterSpacing: 1.5, marginBottom: 20 }}>DEVICE PROTOCOLS</Text>
            <View style={{ gap: 12 }}>
              {Object.entries(presets).map(([id, p]) => (
                <TouchableOpacity
                  ...
                  style={{
                    padding: 16,
                    borderRadius: 14,
                    backgroundColor: preset === id ? '#B37213' : '#1A1A1A',
                    borderWidth: 1,
                    borderColor: preset === id ? '#B37213' : '#333',
                    flexDirection: 'row',
                    alignItems: 'center',
                    justifyContent: 'space-between'
                  }}
                >
                  <Text style={{ color: '#FFF', fontWeight: '600', fontSize: 12 }}>{p.label}</Text>
                  {preset === id && <LucideIcons.Check size={16} color="#FFF" />}
                </TouchableOpacity>
              ))}
            </View>

            <View style={{ height: 1, backgroundColor: '#222', marginVertical: 32 }} />

            <Text style={{ color: '#444', fontWeight: '600', fontSize: 10, letterSpacing: 1.5, marginBottom: 20 }}>ORIENTATION</Text>
            <TouchableOpacity
              onPress={() => setOrientation(isPortrait ? 'landscape' : 'portrait')}
              style={{
                padding: 16,
                borderRadius: 14,
            ...
            >
              <LucideIcons.RotateCcw size={18} color="#B37213" />
              <Text style={{ color: '#FFF', fontWeight: '600', fontSize: 12 }}>SWITCH TO {isPortrait ? 'LANDSCAPE' : 'PORTRAIT'}</Text>
            </TouchableOpacity>
          </View>

          <View>
            <Text style={{ color: '#333', fontSize: 10, fontWeight: '700', letterSpacing: 2 }}>
              SIMULATOR ENGINE v1.0
            </Text>
            <Text style={{ color: '#222', fontSize: 9, fontWeight: '600', marginTop: 4 }}>
              LAGUNA TACTICAL INTERFACE
            </Text>
          </View>
        </View>

        {/* Right Side: Device Preview Area */}
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 40 }}>
          <View style={{
            width: frameWidth,
            height: frameHeight,
            maxHeight: windowHeight - 100,
            maxWidth: windowWidth - 340,
            backgroundColor: '#000',
            borderRadius: 40,
            overflow: 'hidden',
            borderWidth: 12,

...
try {
    ...
    if ($report) {
        ...
        $message = "Your incident report in " . ($report['barangay'] ?? 'your sector') . " has been verified by Tactical Command.";
        $residentMsg = "Tactical Alert: A verified incident in Brgy. " . ($report['barangay'] ?? 'your area') . " requires attention.";

        // Notify Everyone in Brgy (Verified Status)
        NotificationHelper::notify($pdo, "Tactical Alert", $residentMsg, 'residents', $report['barangay'], 'warning', 'emergency', null, ['report_id' => $id]);

        // Notify Reporter specifically
        NotificationHelper::notifyReportUpdate($pdo, array_merge($report, ['id' => $id]), $title, $message, 'success');
    }

...
try {
    ...
    function evaluateRouteSafety($coords, $avoidRings)
    {
        ...
        foreach ($avoidRings as $ring) {
            ...
            // 2. Check segments (Prevent jumping through hazards)
            for ($i = 0; $i < count($coords) - 1; $i++) {
                $p1 = $coords[$i];
                $p2 = $coords[$i + 1];
                $ringCoords = $ring['coords'];
                for ($j = 0; $j < count($ringCoords) - 1; $j++) {
                    if (segmentsIntersect($p1, $p2, $ringCoords[$j], $ringCoords[$j + 1])) {
                        $intersects = true;
                        if ($ring['hard'])
                            $hardBlock = true;
                    }
            ...
            // 3. Distance calculation (Simplified proximity for scoring)
            foreach ($coords as $p) {
                $d = sqrt(pow($p[0] - $ring['coords'][0][0], 2) + pow($p[1] - $ring['coords'][0][1], 2));
                if ($d < $minDist)
                    $minDist = $d;
            }
        }
    ...
    function isUniqueRoute($newRoute, $existingRoutes)
    {
        ...
        // Sample 5 points along the route (20% increments) for higher spatial precision
        $indices = [
            floor(count($newCoords) * 0.20),
            floor(count($newCoords) * 0.40),
            floor(count($newCoords) * 0.60),
            floor(count($newCoords) * 0.80),
            floor(count($newCoords) * 0.50)
        ];
        ...
        foreach ($existingRoutes as $ex) {
            ...
            foreach ($indices as $idx) {
                ...
                if ($dist > 0.002) { // ~200m variance threshold for urban diversity
                    $isDifferent = true;
                    break;
                }
            }
    ...
    // ── Build Avoid Rings & Excludes ──────────────────────────────────────────
    // Map internal profile to allowed_modes keys
    $currentMode = 'car';
    if (strpos($profile, 'walking') !== false || strpos($profile, 'foot') !== false)
        $currentMode = 'walking';
    else if (strpos($profile, 'cycling') !== false || strpos($profile, 'motorcycle') !== false)
        $currentMode = 'motorcycle';
    else if (strpos($profile, 'hgv') !== false || strpos($profile, 'truck') !== false)
        $currentMode = 'truck';
    ...
    foreach ($avoidZones as $zone) {
        ...
        foreach ($rings as $r) {
            ...
            // Mapbox 'exclude' parameter supports points. 
            // For Polygons, we use the centroid to trigger avoidance.
            if (($zone['geometry']['type'] ?? '') === 'Point') {
                $c = $zone['geometry']['coordinates'];
                $excludePoints[] = ['lng' => $c[0], 'lat' => $c[1]];
            } else if (($zone['geometry']['type'] ?? '') === 'Polygon') {
                $coords = $zone['geometry']['coordinates'][0];
                $sumLat = 0; $sumLng = 0;
                foreach ($coords as $pt) { $sumLng += $pt[0]; $sumLat += $pt[1]; }
                $excludePoints[] = [
                    'lng' => $sumLng / count($coords),
    ...
    // NOTE: Mapbox Directions API v5 'exclude' parameter only supports keywords like 'toll', 'motorway'.
    // It DOES NOT support custom points. We will use tactical flanking instead.
    $excludeParam = ""; 
    ...
    // Phase 1: Request Primary & Natural Alternatives with 'exclude' parameter
    $directUrl = "{$baseUrl}/{$startLng},{$startLat};{$endLng},{$endLat}{$baseParams}&alternatives=true&radiuses=1000;1000{$excludeParam}";
    ...
    if (count($allFound) < 3) {
        ...
        // Tactical nudge points (Expanded for better diversity)
        // Tactical nudge points (High-precision surgical offsets)
        $nudgeSpecs = [
            ['at' => 0.10, 'side' => 1, 'push' => 0.10],
            ['at' => 0.10, 'side' => -1, 'push' => 0.10],
            ['at' => 0.25, 'side' => 1, 'push' => 0.18],
            ['at' => 0.25, 'side' => -1, 'push' => 0.18],
            ['at' => 0.33, 'side' => 1, 'push' => 0.35], // New wide flank
            ['at' => 0.33, 'side' => -1, 'push' => 0.35],
            ['at' => 0.50, 'side' => 1, 'push' => 0.45], // Deeper search
            ['at' => 0.50, 'side' => -1, 'push' => 0.45],
            ['at' => 0.66, 'side' => 1, 'push' => 0.35],
            ['at' => 0.66, 'side' => -1, 'push' => 0.35],
            ['at' => 0.75, 'side' => 1, 'push' => 0.18],
            ['at' => 0.75, 'side' => -1, 'push' => 0.18],
            ['at' => 0.90, 'side' => 1, 'push' => 0.10],
            ['at' => 0.90, 'side' => -1, 'push' => 0.10]
        ];
        ...
        foreach ($nudgeSpecs as $spec) {
            ...
            // Use Strategic Nudge points to force the route away from hazards.
            // radiuses=500 gives more room for Car to find a legal road.
            $altUrl = "{$baseUrl}/{$startLng},{$startLat};{$nX},{$nY};{$endLng},{$endLat}{$baseParams}&alternatives=false&radiuses=500;500;500";
    ...
    if (count($allFound) < 10 && $hasAvoid) {
        try {
            ...
            // Analyze the first 5 hazards in the path area
            $hazardsToFlank = array_slice($avoidRings, 0, 5);
            foreach ($hazardsToFlank as $hIdx => $h) {
                ...
                $cX = ($minLng + $maxLng) / 2;
                $cY = ($minLat + $maxLat) / 2;
                // Surgical Buffer: Radius of hazard + ~250m (0.0025 degrees)
                $radius = max($maxLng - $minLng, $maxLat - $minLat) / 2 + 0.0025;

                // Points: 2 Perpendicular "Pushes" + 4 Diagonal Flanks for wider discovery
                $offsets = [
                    [cos($perpBearing1) * $radius, sin($perpBearing1) * $radius],
                    [cos($perpBearing2) * $radius, sin($perpBearing2) * $radius],
                    [cos($perpBearing1 + M_PI / 4) * ($radius * 1.8), sin($perpBearing1 + M_PI / 4) * ($radius * 1.8)],
                    [cos($perpBearing2 - M_PI / 4) * ($radius * 1.8), sin($perpBearing2 - M_PI / 4) * ($radius * 1.8)],
                    [cos($perpBearing1 - M_PI / 4) * ($radius * 1.2), sin($perpBearing1 - M_PI / 4) * ($radius * 1.2)],
                    [cos($perpBearing2 + M_PI / 4) * ($radius * 1.2), sin($perpBearing2 + M_PI / 4) * ($radius * 1.2)]
                ];

                foreach ($offsets as $off) {
                    $px = $cX + $off[0];
                    $py = $cY + $off[1];
                    // Tight Snap: Force Mapbox to find a road within 250m of the safe nudge point
                    $detourUrl = "{$baseUrl}/{$startLng},{$startLat};{$px},{$py};{$endLng},{$endLat}{$baseParams}&radiuses=250;250;250";
    ...
    // --- TACTICAL SELECTION LOGIC (ULTRA-STRICT SAFETY) ---
    // We prioritize perfectly safe routes (Tier 1). 
    // If we have Tier 1 routes, we ONLY use them for all 3 slots if they are unique.
    $finalRoutes = [];
    foreach ($tier1 as $candidate) {
        if (count($finalRoutes) >= 3) break;
        if (isUniqueRoute($candidate, $finalRoutes)) $finalRoutes[] = $candidate;
    }

    // Only if we have fewer than 3 safe routes, we add from Tier 2 (Clips)
    if (count($finalRoutes) < 3) {
        foreach ($tier2 as $candidate) {
            if (count($finalRoutes) >= 3) break;
            if (isUniqueRoute($candidate, $finalRoutes)) $finalRoutes[] = $candidate;
        }
    }

    // Only as absolute last resort, we add from Tier 3 (Danger)
    if (count($finalRoutes) < 3) {
        foreach ($tier3 as $candidate) {
            if (count($finalRoutes) >= 3) break;
            if (isUniqueRoute($candidate, $finalRoutes)) $finalRoutes[] = $candidate;
        }
    }
    ...
    $endInHazard = false;
    foreach ($avoidRings as $ring) {
        if (pointInPolygon($startLng, $startLat, $ring['coords']))
            $startInHazard = true;
        if (pointInPolygon($endLng, $endLat, $ring['coords']))
            $endInHazard = true;
    }
